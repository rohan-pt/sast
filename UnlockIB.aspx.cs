...
{
    public partial class UnlockIB : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                SqlCommand cmd1 = new SqlCommand("SELECT C.CustNo, UPPER(LTRIM(RTRIM(C.Longname))) AS CustName, IB.Status, " +
                        "CASE WHEN IB.Status = 1 THEN 'ACTIVE' WHEN IB.Status = 2 THEN 'BLOCKED' WHEN IB.Status = 7 THEN 'NEW LOGIN' END AS LoginIDStatus, M.MobileNo " +
                        "FROM D002011 IB JOIN D009011 C ON IB.CustNo = C.CustNo " +
                        "LEFT JOIN D350078 M ON CAST(M.CustNo AS VARCHAR) = CAST(C.CustNo AS VARCHAR) " +
                        "WHERE IB.CustNo = '" + Request.Form["txtcardnumber"].ToString() + "' " +
                        "ORDER BY C.CustNo", con);
                //cmd1.Parameters.AddWithValue("@RRN", txtcardnumber.Value);
                SqlDataReader usernameRdr = null;

                usernameRdr = cmd1.ExecuteReader();
                int i = 0;
                string name = "", custnum = "", Mobnum = "", Stat = "", logstat = "";
                int flag = 0;
                ...
                if (usernameRdr.HasRows == true)
                {
                    ...
                    {
                        name = usernameRdr["CustName"].ToString();
                        custnum = usernameRdr["CustNo"].ToString();
                        Mobnum = usernameRdr["MobileNo"].ToString();
                        Stat = usernameRdr["Status"].ToString();
                        logstat = usernameRdr["LoginIDStatus"].ToString(); 
                    }
                    tdname.InnerText = name;
                   tdcustnum.InnerText = custnum;
                    tdmobno.InnerText = Mobnum;
                    tdstatus.InnerText = Stat;                  


                    if (logstat == "ACTIVE")
                    {
                        tdlogstat.InnerText = logstat;
                        tdlogstat.Style.Add("background-color", "#4ed321");
                        tdlogstat.Style.Add("color", "#ffffff");
                    }
                    else
                    {
                        tdlogstat.InnerText = logstat;
                        tdlogstat.Style.Add("background-color", "#d32121");
                        tdlogstat.Style.Add("color", "#ffffff");
                        flag = 1;
                    }
                    ...
                    if(flag==0)
                    {
                        lblmsg.Text = "Card is Active.";
                        btnunblock.Visible = false;
                    }
                    else
                    {
                        lblmsg.Text = "Card is not in Active Mode.";
                ...
                {
                    lblmsg.Text = "There is nothing like this in System.";
                    lblmsg.Visible = true;
            ...
            {
                lblmsg.Text = rr.Message.ToString();
                lblmsg.Visible = true;
            }
        }
        ...
        {
            Response.Redirect("UnlockIB.aspx");
        }
        ...
        protected void btnunblock_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                SqlCommand CMD = new SqlCommand("UPDATE D002011 SET Status=1, DbtrLupdMk=" + x + ", DbtrLupdCk=" + x + ", DbtrLupdMd=CONVERT(date, getdate(), 23), " +
                    "DbtrLupdCd=CONVERT(date, getdate(), 23) WHERE CustNo='" + Request.Form["txtcardnumber"].ToString() + "' ", cnn);
                int ret = CMD.ExecuteNonQuery();

                //MessageBox.Show(txtLoginID.Text.ToString() + " Un-Blocked.");
                //InternetBankingUnblockCust fr = new InternetBankingUnblockCust();
                //fr.Show();
                //this.Hide();
                if(ret==1)
                {
                    lblmsg.Text = "Customer successfully unblock for IB";
                }
                else
                {
                    lblmsg.Text = "Problem in updation: " + ret;
                }
            }
            catch (Exception rr)
            {
